# Vibi Desktop Companion Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Entregar una aplicación de Windows que despierte con «Vibi», muestre la cabeza 3D y mantenga una conversación de voz hasta una despedida o clic.
...
- El detector de «Vibi» debe funcionar sin red y no guardar audio.
- La campanita solo suena al iniciar la conversación, no entre turnos.
- La conversación termina con «adiós Vibi», «gracias Vibi» o un clic.
...
- [ ] **Step 2:** Normalizar la transcripción con Unicode y detectar exactamente `adios vibi` o `gracias vibi` antes de invocar `procesar_mensaje`.
...
- [ ] **Step 1:** Implementar captura mono PCM16 a 16 kHz con `sounddevice.RawInputStream` y `KaldiRecognizer(model, 16000, '["vibi", "[unk]"]')`.
- [ ] **Step 2:** Detectar «vibi» en resultados parciales/finales con debounce de dos segundos y escribir un evento `wake` por línea.
...
**Interfaces:**
- Produces events: `vibi://wake`, `vibi://listener-error`
...
- [ ] **Step 2:** Implementar estados `sleeping`, `listening`, `thinking`, `speaking`, `error` reutilizando `VibiFace` y `startVoiceCapture`.
