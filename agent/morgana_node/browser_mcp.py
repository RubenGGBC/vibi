...
def arrancar(
    ...
    if escuchando(puerto):
        if not hosts_permitidos or acepta_host(puerto, hosts_permitidos.split(",")[0]):
            return {
                "estado": "ok",
                "puerto": puerto,
                "arrancado_ahora": False,
                "pid": _proceso.pid if nuestro else None,
            }
        if not nuestro:
            # Ocupado por algo que no hemos lanzado y que no atiende a quien va
            # a llamar. Matarlo a ciegas sería peor que decirlo.
            raise BrowserMCPError(
                f"El puerto {puerto} ya está ocupado por un servidor que "
                f"rechaza las peticiones de «{hosts_permitidos}». Ciérralo o "
                f"elige otro puerto."
            )
        # Nuestro pero mal configurado: viene de una versión anterior de esto.
        _terminar(_proceso)
    elif nuestro:
    ...
    proceso = _lanzar(
        comando(puerto, navegador, destino, host, hosts_permitidos), destino
    )
    ...
    while time.time() < limite:
        ...
        if proceso.poll() is not None:
            raise BrowserMCPError(
                ...
                f"{proceso.returncode}). Comprueba que Node.js funciona y que "
                f"el navegador «{navegador}» está instalado."
            )
        time.sleep(SONDEO)
...
def parar(puerto: int = PUERTO_POR_DEFECTO) -> dict:
    """Cierra el servidor, y con él la ventana del navegador.

    Solo puede cerrar el que lanzó este agente. Uno heredado de una ejecución
    anterior sigue siendo suyo, y matarlo a ciegas por el puerto podría
    llevarse por delante otra cosa que estuviera escuchando ahí.
    """
    global _proceso
